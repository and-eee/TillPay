            ...
            foreach ($available_gateways as $gateways_id => $gateways) {
                if ($gateways_id !== $this->order->get_payment_method()) {
                    unset($available_gateways['jetpack_custom_gateway']);
                      ...
                      unset($available_gateways['airwallex_card']);
...
//        $this->order->update_status('pending', __('Awaiting payment', 'woocommerce'));  // AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
        $this->order->add_order_note(                                       // AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            __('Payment process initiated', 'woocommerce') // Add this line to add a note   AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
        );
...
//          ->setCompany($this->order->get_billing_company())
            ->setCompany(substr($this->order->get_billing_company(), 0, 55))    // Limit company name to 55 characters AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA            
        ...
        $transaction->setTransactionId($orderTxId)
            ->setmerchantMetaData($this->order->get_id())  // AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            ->setdescription("Order #{$this->order->get_id()}")  // AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA            
...
// ----------------------- UPDATED CODE START ---------------------------- AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA

//      $this->order->update_status('failed', __('Payment failed or was declined', 'woocommerce'));  
        $this->order->add_order_note( 
            __('Payment failed or was declined', 'woocommerce')
        );
        WC()->mailer()->get_emails()['WC_Email_Failed_Order']->trigger($this->order->get_id());

// ----------------------- UPDATED CODE END ---------------------------- AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA

//      wc_add_notice(__('Payment failed or was declined', 'woocommerce'), 'error');
//      wc_add_notice(__('<p style="color: RED"><br><br><b>Payment failed or was declined</b><br>You can try again or call us on 02 9640 0366 to find out more</p>', 'woocommerce'), 'error');  //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
// -------------------------- UPDATED CODE START -----------------------------  AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA

//                  $this->order->update_status('failed', __('Error', 'woocommerce'));
                    $this->order->add_order_note(
                        __('Error during payment process', 'woocommerce')
                    );
                        WC()->mailer()->get_emails()['WC_Email_Failed_Order']->trigger($this->order->get_id());

// -------------------------- UPDATED CODE END -----------------------------  AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
